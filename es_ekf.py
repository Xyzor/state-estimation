...
C_li = np.array([[0.99376, -0.09722, 0.05466], [0.09971, 0.99401, -0.04475], [-0.04998, 0.04992, 0.9975]])
...
P = np.zeros([imu_f.data.shape[0], 9, 9])  # covariance matrices at each timestep
...
P[0] = np.eye(9)  # covariance of estimate
...
# a function for it.
####
def measurement_update(sensor_var, P_check, y_k, p_check, v_check, q_check):
    # 3.1 Compute Kalman Gain
    R = sensor_var * np.eye(3)
    S = H.dot(P_check.dot(H.T)) + R
    K = P_check.dot(H.T.dot(np.linalg.inv(S)))

    # 3.2 Compute error state
    error_est = K.dot(y_k - p_check)

    # 3.3 Correct predicted state
    p_hat = p_check + error_est[:3]
    v_hat = v_check + error_est[3:6]
    q_hat = Quaternion(axis_angle=error_est[6:]).quat_mult(q_check)

    # 3.4 Compute corrected covariance
    P_hat = (np.eye(9) - K.dot(H)).dot(P_check)

    return p_hat, v_hat, q_hat, P_hat
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k - 1] + delta_t * v_est[k - 1] + 0.5 * (delta_t ** 2) * (C_n.dot(imu_f.data[k - 1]) + g)
    ...
    P[k] = F.dot(P[k - 1]).dot(F.T) + L.dot(Q).dot(L.T)

    # 3. Check availability of GNSS and LIDAR measurements
    for i in range(len(gnss.t)):
        if abs(gnss.t[i] - imu_f.t[k]) < 0.01:
            p_est[k], v_est[k], q_est[k], P[k] = measurement_update(
                var_gnss, P[k], gnss.data[i], p_est[k], v_est[k], q_est[k]
            )
    for i in range(len(lidar.t)):
        if abs(lidar.t[i] - imu_f.t[k]) < 0.01:
            p_est[k], v_est[k], q_est[k], P[k] = measurement_update(
                var_lidar, P[k], lidar.data[i], p_est[k], v_est[k], q_est[k]
            )
...
P_diag_std = np.sqrt(np.diagonal(P, axis1=1, axis2=2))

titles = ["x", "y", "z", "x rot", "y rot", "z rot"]
for i in range(3):
    ax[0, i].plot(range(num_gt), gt.p[:, i] - p_est[:num_gt, i])
    ax[0, i].plot(range(num_gt), 3 * P_diag_std[:num_gt, i], "r--")
    ax[0, i].plot(range(num_gt), -3 * P_diag_std[:num_gt, i], "r--")
    ax[0, i].set_title(titles[i])

for i in range(3):
    ax[1, i].plot(range(num_gt), gt.r[:, i] - p_est_euler[:num_gt, i])
    ax[1, i].plot(range(num_gt), 3 * P_diag_std[:num_gt, i + 6], "r--")
    ax[1, i].plot(range(num_gt), -3 * P_diag_std[:num_gt, i + 6], "r--")
